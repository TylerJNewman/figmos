...
import {
  GETTweetsSearchRecentResponse,
  GETListsIdtweetsResponse,
  GETListsIdTweetsRoute,
} from 'twitter-types'
...
export const getTwitterHotNews = async (): Promise<Link[]> => {
  ...
  return data.data.map(tweet => ({
    ...
    link: getUrlsWithoutIndices(tweet.text)?.length
      ? getUrlsWithoutIndices(tweet.text)[0]
      : '',
